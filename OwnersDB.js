...
app.post('/create_owner', (req, res) => {
    ...
    console.log("Owner information to be inserted:", ownerInfo); // Log the owner information
    const insertQuery = `INSERT INTO owners_information (name, surname, id_number, email, phone_number, gender, password) VALUES (?, ?, ?, ?, ?, ?, ?)`;
    connection.query(insertQuery, [ownerInfo.name, ownerInfo.surname, ownerInfo.id_number, ownerInfo.email, ownerInfo.phone_number, ownerInfo.gender, ownerInfo.password], (err, result) => {
        if (err) {
            console.error("Error creating owner:", err);
            return res.status(500).json({ error: "Failed to create owner" });
        }
        console.log("Owner created successfully");
        res.status(200).json({ message: "Owner created successfully", ownerId: result.insertId });
    });
});
...
app.post('/upload', upload.single("ID_docs"), (req, res) => {
    ...
    const insertQuery = "INSERT INTO documents (owner_id, document_name, document_data) VALUES (?, ?, ?)";
    connection.query(insertQuery, [ownerId, documentName, documentData], (err, result) => {
        if (err) {
...
app.patch(`/update_owner/:id`, async (req, res) => {
    try {
      const { id } = req.params;
      const { name, surname, id_number, email, phone_number, gender, password } = req.body;
      
      // Check if at least one field to update is provided
      if (!(name || surname || id_number || email || phone_number || gender || password)) {
      ...
      const updateQuery = `UPDATE owners_information SET ${updateFields.join(', ')} WHERE id = ?`;
      updateValues.push(id);
  
      // Execute the update query
      const [update] = await connection.promise().query(updateQuery, updateValues);
